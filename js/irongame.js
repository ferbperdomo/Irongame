const irongame = {
    ...
    document.addEventListener("click", (event) => {
      ...
      this.bullets.push(
        new Bullets(
          ...
          event.clientX,
          event.clientY,
          this.player.playerSize.w,
          this.player.playerSize.h,
          speed,
  ...
  clearBullets() {
    this.bullets = this.bullets.filter(
      (bullet) =>
        this.gameSize.w > bullet.bulletPos.x &&
        bullet.bulletPos.x > 0 &&
        this.gameSize.h > bullet.bulletPos.y &&
        bullet.bulletPos.y > 0
    );
  },
  ...
  createPythonEnemy() {
    ...
    const newEnemy = new PythonEnemy(
      this.ctx,
      this.getRandomX("python"),
      this.getRandomY("python"),
      80,
      80,
      this.gameLimits
    );
  ...
  createOctoEnemy() {
    const newEnemy = new OctoEnemy(
      this.ctx,
      this.getRandomX("octo"),
      this.getRandomY("octo"),
      80,
      80,
      this.gameLimits
    );
  ...
  getRandomY(enemy) {
    let size = 80;
    if (enemy === "python" && this.pythonEnemies.length > 0)
      size = this.pythonEnemies[0].pythonEnemySize.h;
    else if (enemy === "octo" && this.octoEnemies.length > 0)
      size = this.octoEnemies[0].octoEnemySize.h;
    if (size) {
      const random = Math.floor(
        Math.random() * (this.gameLimits.b - size - this.gameLimits.t) +
          this.gameLimits.t
      );
  ...
  getRandomX(enemy) {
    let size = 80;
    if (enemy === "python" && this.pythonEnemies.length > 0)
      size = this.pythonEnemies[0].pythonEnemySize.w;
    else if (enemy === "octo" && this.octoEnemies.length > 0)
      size = this.octoEnemies[0].octoEnemySize.w;
    if (size) {
      const random = Math.floor(
        Math.random() * (this.gameLimits.r - size - this.gameLimits.l) +
          this.gameLimits.l
      );
